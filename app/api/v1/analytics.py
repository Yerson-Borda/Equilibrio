...
class DateRange(BaseModel):
    ...
    @classmethod
    def validate_range(cls, start_date: date, end_date: date):
...
def get_monthly_comparison(
        month: str = Query(..., description="Selected month in YYYY-MM format. Will compare with previous month automatically"),
        current_user: User = Depends(get_current_user),
        db: Session = Depends(get_db)
):
    """Get category-wise comparison between selected month and previous month (automatically calculated)"""
...
def get_spending_trends(
    ...
    try:
        ...
        for result in monthly_data:
            ...
            if key not in monthly_totals:
                monthly_totals[key] = {
                    ...
                    "display_name": datetime(int(result.year), int(result.month), 1).strftime("%b %Y")
                }
        ...
        while current <= end_date.replace(day=1):
            ...
            existing_month = monthly_totals.get(month_key, {
                "year": current.year,
                "month": current.month,
                "total_spent": 0.0,
                "total_income": 0.0,
        ...
        average_monthly_spent = total_spent / len(complete_monthly_data) if complete_monthly_data else 0
